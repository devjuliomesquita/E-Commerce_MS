...
public class PurchaseOrderServiceImpl {
    ...
    public Integer create(OrderRequest request) {
        ...
        if (purchaseResponses.isEmpty()) {
            throw new CommunicationBetweenServicesException("Erro ao processar os produtos.");
        }

        PurchaseOrder purchaseOrder = this.purchaseOrderRepository.save(this.purchaseOrderMapper.toEntity(request));

        request.products().forEach(p -> this.orderLineService.create(
                new OrderLineRequest(null, purchaseOrder.getId(), p.productId(), p.quantity())));
